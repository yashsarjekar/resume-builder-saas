...
function ResumePickerModal({
  ...
  job: Job;
  resumes: { id: number; title: string; updated_at: string }[];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <div className="relative bg-white w-full sm:max-w-md sm:rounded-2xl rounded-t-3xl shadow-2xl overflow-hidden">
        {/* Header */}
        <div className="px-6 pt-6 pb-4 border-b border-gray-100">
          ...
          <div className="flex items-center gap-3 pr-8">
            ...
            <div>
              <h3 className="font-black text-gray-900 text-base">Select Resume to Match</h3>
              <p className="text-xs text-gray-500 mt-0.5 line-clamp-1">Matching against: {job.title}</p>
            </div>
          </div>
        </div>
        {/* Resume list */}
        <div className="p-4 space-y-2 max-h-80 overflow-y-auto">
          {resumes.map((resume) => (
            <button
              ...
              className="w-full flex items-center gap-3 p-3.5 rounded-xl border border-gray-200 hover:border-indigo-400 hover:bg-indigo-50 transition-all text-left group"
            >
              <div className="w-9 h-9 rounded-lg bg-gradient-to-br from-indigo-500 to-purple-600 flex items-center justify-center flex-shrink-0 shadow-sm">
                <svg className="w-4 h-4 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                </svg>
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-gray-900 text-sm truncate group-hover:text-indigo-700">{resume.title}</p>
                <p className="text-xs text-gray-400 mt-0.5">
                  Updated {new Date(resume.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
                </p>
              </div>
              <svg className="w-4 h-4 text-gray-300 group-hover:text-indigo-500 flex-shrink-0 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="px-4 pb-4 pt-2">
          <p className="text-center text-xs text-gray-400">Select the resume you want to check against this job</p>
        </div>
      </div>
    </div>
  );
...
export default function JobsPage() {
  ...
  const handleAtsMatch = (job: Job) => {
    if (resumes.length === 0) return;
    if (resumes.length === 1) {
      runAtsMatch(job, resumes[0].id);
    } else {
    ...
    <>
      ...
      {resumePickerJob && (
        <ResumePickerModal
          job={resumePickerJob}
          resumes={resumes}
          onSelect={(resumeId) => runAtsMatch(resumePickerJob, resumeId)}
          onClose={() => setResumePickerJob(null)}
        />
      )}
