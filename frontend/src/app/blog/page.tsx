...
// ISR — revalidate every hour so new published blogs appear automatically
...
function BlogCard({ post, featured = false }: { post: BlogPost; featured?: boolean }) {
  const gradient = categoryGradients[post.category] ?? 'from-indigo-600 to-blue-600';
  const color    = categoryColors[post.category]    ?? categoryColors['resume-tips'];
  const icon     = categoryIcons[post.category]     ?? categoryIcons['resume-tips'];

  return (
    <Link href={`/blog/${post.slug}`} className="group glass-card rounded-xl overflow-hidden block h-full">
      {/* Gradient thumbnail */}
      <div className={`relative bg-gradient-to-br ${gradient} overflow-hidden ${featured ? 'h-44' : 'h-36'}`}>
      ...
      {/* Content */}
      <div className="p-5">
        <span className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-medium mb-3 ${color}`}>
...
export default async function BlogPage() {
  ...
  const [allPosts, featuredPosts, resumeTips, interviewPrep, careerAdvice] = await Promise.all([
    fetchAllPosts(),
    fetchFeaturedPosts(),
    fetchPostsByCategory('resume-tips'),
    fetchPostsByCategory('interview-prep'),
    fetchPostsByCategory('career-advice'),
  ]);
      ...
      <div className="bg-[#050816] min-h-screen">
        ...
        <section className="relative py-20 overflow-hidden">
          ...
          <div className="container mx-auto px-4 max-w-6xl relative z-10 text-center">
            ...
            <h1 className="text-4xl md:text-5xl font-black text-white mb-4 tracking-tight">Career Blog</h1>
        ...
        <section className="py-16 relative overflow-hidden">
          ...
          <div className="container mx-auto px-4 max-w-3xl text-center relative z-10">
            ...
            <p className="text-gray-400 mb-8 text-lg">Put these tips into action with our AI-powered resume builder.</p>
